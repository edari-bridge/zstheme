...
render() {
    ...
    if [[ "$ANIMATION_MODE" == "rainbow" ]]; then
        ...
        loc_content="${c0}${ICON_BRANCH} ${BRANCH:-branch}    "
        loc_content="${loc_content}${c1}${ICON_TREE} ${WORKTREE:-worktree}    "
        loc_content="${loc_content}${c2}${ICON_DIR} ${DIR_NAME}"
    else
    ...
    if [[ "$ANIMATION_MODE" == "rainbow" ]]; then
        ...
        ses_content="${c9}${ICON_MODEL} ${MODEL}"
    else
        # lsd와 static 모두 동일한 글자색
